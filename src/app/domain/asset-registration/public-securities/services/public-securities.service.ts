...
export class PublicSecuritiesService {
  ...
  private readonly baseUrl = '/api/public-securities';
  ...
  /**
   * Busca lista de títulos públicos
   */
  getAll(filters?: PublicSecurityFilters): Observable<PublicSecurityRecord[]> {
    // TODO: Implementar chamada real à API
    // return this.http.get<PublicSecurityRecord[]>(this.baseUrl, { params: filters as any });
    return of(this.mockData);
  }

  /**
   * Busca título público por ID
   */
  getById(id: number): Observable<PublicSecurityRecord | undefined> {
    // TODO: Implementar chamada real à API
    // return this.http.get<PublicSecurityRecord>(`${this.baseUrl}/${id}`);
    return of(this.mockData.find(item => item.id === id));
  }

  /**
   * Busca título público por código do ativo
   */
  getByCodigo(codigo: string): Observable<PublicSecurityRecord | undefined> {
    // TODO: Implementar chamada real à API
    // return this.http.get<PublicSecurityRecord>(`${this.baseUrl}/codigo/${codigo}`);
    return of(this.mockData.find(item => item.codigo === codigo));
  }

  /**
   * Busca detalhes completos de um título público
   */
  getBondDetails(id: number): Observable<PublicBondCharacteristic | null> {
    // TODO: Implementar chamada real à API
    // return this.http.get<PublicBondCharacteristic>(`${this.baseUrl}/bond/${id}`);
    return of(null);
  }

  /**
   * Busca tipos de títulos públicos disponíveis
   */
